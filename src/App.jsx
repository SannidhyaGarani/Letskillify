...
const CoursesList = lazy(() => import("./components/Courses/CoursesList.jsx"));
const TopScroll = lazy(() => import("./components/TopScroll.jsx"));
...
function App() {
  ...
  React.useEffect(() => {
    // Ensure preloader stays until all resources are loaded
    const onLoad = () => {
      // Small delay for smooth transition
      setTimeout(() => setBootLoading(false), 500);
    };
    ...
    if (document.readyState === "complete") {
      ...
      // Fallback in case window load event doesn't fire as expected
      const t = setTimeout(onLoad, 5000);
      return () => {
  ...
  return (
    <>
      <div>
        <MainProvider>
          <LeadPopup />
          <Suspense fallback={<div className="FIrstLoading"></div>}>
            <TopScroll />
            <Navbar />
            <HelmetProvider>
              <Routes>
